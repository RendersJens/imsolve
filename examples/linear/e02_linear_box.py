...
angles = np.linspace(0,np.pi,30) # sparse view
...
x_SIRT = SIRT(A, b, bounds=(0, np.inf), verbose=True, max_iter=20)
x_BBLS = BBLS(A, b, bounds=(0, np.inf), verbose=True, max_iter=20)

# CGLS does not support non-negativity constraint
# so we project on the non-negative orthant afterwards
x_CGLS = CGLS(A, b, verbose=True, max_iter=20)
x_CGLS_clip = np.clip(x_CGLS, 0, np.inf)
...
plt.figure()
plt.title("clipped CGLS, iteration 20")
plt.imshow(x_CGLS_clip.reshape(n_pix, n_pix), cmap="gray")
